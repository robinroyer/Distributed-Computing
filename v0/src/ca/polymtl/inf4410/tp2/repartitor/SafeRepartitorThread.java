...
public class SafeRepartitorThread extends Thread {
	 ...
	 * @param server
	 *            CalculousServerInterface server handle by the thread
	 * @param calculations
	 *            shared String ArrayList of calculous
	 * @param calculationsSemaphore
	 *            Semaphore protecting calculations
	 * @param globalResult
	 *            Array containging [0] shared result, [1] operations allready
	 *            proceed
	 * @param globalResultLock
	 *            Semaphore protecting globalResult
	 */
	public SafeRepartitorThread(Repartitor repart, CalculousServerInterface server, ArrayList<String> calculations,
			Semaphore calculationsSemaphore, int[] globalResult, Semaphore globalResultLock) {
	...
	/**
	 * loop over calculous, safely take operations and proceed them until shared
	 * boolean threadsShouldContinue is set to true
	 */
	@Override
	public void run() {
		int actualResult;
	 ...
	 * 
	 * @param server
	 *            CalculousServerInterface that will proceed the calculation
	 * @param operations
	 *            array of string containing the operations
	 * @return result proceed by the server (it can be malicious)
	 * @throws RemoteException
	 *             Rmi exception
	 * @throws OverloadedServerException
	 *             exception thrown if we sent too much operations to the server
	 */
	 ...
	 *            number of operation proceed by the server
	...
	/**
	 * Safe Sending back calculous operation taken by the thread to the shared
	 * calculous list
	 * 
	 * @throws InterruptedException
	 */
